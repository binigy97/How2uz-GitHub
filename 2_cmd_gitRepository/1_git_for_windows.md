...
#### 실패
- 환경변수를 바르게 추가했는데 git 명령어를 사용할 수 없다면?  
  > cmd를 새로 열어서 테스트하기  
- 어제까진 잘 연결됐던 깃이 오늘은 연결이 안 된다면?  
  > 3)과정 중, 해당 app-.. 하위에 깃 실행 프로그램이 있는지 확인하기  
  > (아마 깃이 업데이트되어) 기존의 경로에 git.exe 파일이 없다면 시스템 속성의 환경변수(PATH)도 재설정해주어야 한다.  
